...
void printTimings(struct Timer* t, int myRank, int NPEs)
{
    ...
    if(!myRank) {
        printf("%f;%f;%f;%f;%f;%f\n", maxT.init, maxT.update, maxT.comm, maxT.evolve, maxT.save, maxT.total);
        printf("%f;%f;%f;%f;%f;%f\n", avgT.init/NPEs, avgT.update/NPEs, avgT.comm/NPEs, avgT.evolve/NPEs, avgT.save/NPEs, avgT.total/NPEs);
    }
}
